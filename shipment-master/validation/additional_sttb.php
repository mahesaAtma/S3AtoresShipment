...
function saveShipment($data, $dbconn, $nowDate, $nowDatetime) {
    ...
    $detail1Validate = insertTrxShipmentEntryD1($data, $dbconn, $nowDate, $nowDatetime, 'XX-TESTING-XX', $orderPickup, $customer, $user);
    if (!$detail1Validate['success']) {
        return ['success' => false, 'message' => $detail1Validate['message']];
    }
    
    $detail1ServiceValidate = insertTrxShipmentEntryD1Service($data, $dbconn, $nowDatetime, $orderPickup, 9999999999, $user);
    if (!$detail1ServiceValidate['success']) {
        return ['success' => false, 'message' => $detail1ServiceValidate['message']];
    }
    
    $detail2Validate = insertTrxShipmentEntryD2($data, $dbconn, $nowDatetime, 'XX-TESTING-XX', $orderPickup, $user);
    if (!$detail2Validate['success']) {
        return ['success' => false, 'message' => $detail2Validate['message']];
    }

    $detail3Validate = insertTrxShipmentEntryD3($data, $dbconn, $nowDatetime, 'XX-TESTING-XX', $orderPickup, $user);
    if (!$detail3Validate['success']) {
        return ['success' => false, 'message' => $detail3Validate['message']];
    }

    $detail3PackingValidate = insertTrxShipmentEntryD3Packing($data, $dbconn, $nowDatetime, 'XX-TESTING-XX', $orderPickup, $user);
    ...
    $shipmentEntryQueriesD1 = insertTrxShipmentEntryD1($data, $dbconn, $nowDate, $nowDatetime, $noShipment, $orderPickup, $customer, $user);
    $shipmentEntryD1ID = insertQueries($dbconn, $shipmentEntryQueriesD1['queries'], true)['lastId'];
    
    $statusSTTBQueries = insertStatusSTTB($data, $noShipment, $nowDatetime, $orderPickup, $user);
    insertQueries($dbconn, $statusSTTBQueries['queries']);

    $shipmentEntryQueriesD1Service = insertTrxShipmentEntryD1Service($data, $dbconn, $nowDatetime, $orderPickup, $shipmentEntryD1ID, $user);
    insertQueries($dbconn, $shipmentEntryQueriesD1Service['queries']);

    $shipmentEntryQueriesD2 = insertTrxShipmentEntryD2($data, $dbconn, $nowDatetime, $noShipment, $orderPickup, $user);
    insertQueries($dbconn, $shipmentEntryQueriesD2['queries']);

    $shipmentEntryQueriesD3 = insertTrxShipmentEntryD3($data, $dbconn, $nowDatetime, $noShipment, $orderPickup, $user);
    insertQueries($dbconn, $shipmentEntryQueriesD3['queries']);

    $shipmentEntryQueriesD3Packing = insertTrxShipmentEntryD3Packing($data, $dbconn, $nowDatetime, $noShipment, $orderPickup, $user);
